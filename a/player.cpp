...
const int Player::getWidth() const { return 60; }
...
void Player::setCamZoom(float zoom) { this->cam.zoom = zoom; }
...
void Player::Draw(int frame)
{
	if (vel.y < 0)
	{
		...
		Rectangle frameR = { 60 * frame, 0, 60 * sgn(vel.x), texture_jump.height };

		DrawTextureRec(texture_jump, frameR, { pos.x - 30, pos.y }, RAYWHITE);
	}
	else if (abs(vel.x) > 10)
	{
		if (sgn(vel.x) - 1)
			frame = texture_walk.width - frame;
		Rectangle frameR = { 60 * frame, 0, 60 * sgn(vel.x), texture_walk.height };

		DrawTextureRec(texture_walk, frameR, { pos.x - 30, pos.y }, RAYWHITE);
	}
	else
		DrawTextureV(texture_idle, { pos.x - 30, pos.y }, RAYWHITE);
}